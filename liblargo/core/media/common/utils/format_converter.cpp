...
static ffmpeg::sample_format_t get_sample_format(AVCodecID codec_id)
{
    auto formats = ffmpeg::codec_info_t::supported_audio_formats(codec_id);
    return formats.empty()
            ? ffmpeg::sample_format_none
            : formats.front();
}
...
static ffmpeg::pixel_format_t get_pixel_format(AVCodecID codec_id)
{
    auto formats = ffmpeg::codec_info_t::supported_video_formats(codec_id);
    return formats.empty()
            ? ffmpeg::pixel_format_none
            : formats.front();
}
...
{
    if (video::video_info_t::is_encoded(pixel_format))
    {
        static ffmpeg::pixel_formats_t supported_formats;
        if (supported_formats.empty())
        {
            supported_formats = ffmpeg::codec_info_t::supported_video_formats(to_ffmpeg_video_codec(pixel_format));
        }
...
{
    if (audio::audio_info_t::is_encoded(sample_format))
    {
        static ffmpeg::sample_formats_t supported_formats;
        if (supported_formats.empty())
        {
            supported_formats = ffmpeg::codec_info_t::supported_audio_formats(to_ffmpeg_audio_codec(sample_format));
        }
