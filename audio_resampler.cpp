...
static std::size_t rescaling_sample(const Tin& input_sample, Tout& output_sample)
{
    output_sample = static_cast<Tout>((static_cast<double>(input_sample) / static_cast<double>(Tin_max)) * static_cast<double>(Tout_max));
    return sizeof(Tout);
}

template<typename Tin>
static std::size_t rescaling_sample(const Tin& input_sample, void* output_sample, std::size_t output_sample_size)
{
    ...
    {
        case 8:
            result = rescaling_sample(input_sample, *static_cast<std::int8_t*>(output_sample));
        break;
        case 16:
            result = rescaling_sample(input_sample, *static_cast<std::int16_t*>(output_sample));
        break;
        case 32:
            result = rescaling_sample(input_sample, *static_cast<std::int32_t*>(output_sample));
    }

    return result;
}

std::size_t rescaling_sample(const void* input_sample, std::size_t input_sample_size, void* output_sample, std::size_t output_sample_size)
{
    std::size_t result = 0;
    ...
    else
    {
        ...
        {
            case 8:
                result = rescaling_sample(*static_cast<const std::int8_t*>(input_sample), output_sample, output_sample_size);
            break;
            case 16:
                result = rescaling_sample(*static_cast<const std::int16_t*>(input_sample), output_sample, output_sample_size);
            break;
            case 32:
                result = rescaling_sample(*static_cast<const std::int32_t*>(input_sample), output_sample, output_sample_size);
        }
    }
            ...
            && input_size > 0)
    {
        ...
        if (input_format != output_format)
        {
            ...
            for (auto out_idx = 0; out_idx < output_size; out_idx += output_sample_size)
            {
                ...
                result += reample_utils::rescaling_sample(input_ptr + in_idx,
